"""Hyperbolic layers.
url: https://github.com/HazyResearch/hgcn/blob/master/layers/hyp_layers.py
"""
import math
...
from .attn_layers import DenseAtt
...
class HyperbolicGraphConvolution(nn.Module):
    ...
    def __init__(self, manifold, in_features, out_features, c_in, c_out, dropout, act, use_bias, use_att, local_agg):
        ...
        self.linear = LorentzLinear(manifold, in_features, out_features, use_bias, dropout)
    ...
    def forward(self, input):
        ...
        # h = self.hyp_act.forward(h)
        output = h, adj  # previously: output = h,adj
        return output
...
class HypLinear(nn.Module):
    ...
    def __init__(self, manifold, in_features, out_features, c, dropout, use_bias):
        ...
        self.manifold = Hyperboloid() if manifold == 'Hyperboloid' else None  # adaptation for Hyperboloid manifold
...
class HypAgg(Module):
    ...
    def __init__(self, manifold, c, in_features, dropout, use_att, local_agg):
        ...
        self.manifold = Lorentz() if manifold == 'Hyperboloid' else None  # adaptation for Hyperboloid manifold
        self.c = c
...
'''
url: https://github.com/wuzhanjie123/DSRL/blob/main/layers/hyp_layers.py#L360
...
class LorentzLinear(nn.Module):
    ...
    def forward(self, x):
        if self.nonlin is not None:
            x = self.nonlin(x)
        x = self.weight(self.dropout(x))  #(Wx,v)
