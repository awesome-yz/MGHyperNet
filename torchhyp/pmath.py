...
def _mobius_add(x, y, c):
    ...
    # print("x2.isnan():", torch.isnan(x2).any())
    y2 = y.pow(2).sum(dim=-1, keepdim=True)
    # print("y2.isnan():", torch.isnan(y2).any())
    xy = (x * y).sum(dim=-1, keepdim=True)
    # print("xy.isnan():", torch.isnan(xy).any())
...
def clip_euclidean_embedding(x, r):
    '''
    Function from Clipped Hyperbolic Classifiers Are Super-Hyperbolic Classifiers
    Guo et al.
    ...
    '''
    # import pdb; pdb.set_trace()
    abs_x = torch.linalg.vector_norm(x,ord=1)
    clipped_x = x * min(1.0, float(r/abs_x))
    return clipped_x

def mobius_weighted_sum(weights, values):
    """
    Weighted sum in Poincaré ball using Möbius scalar multiplication and Möbius addition.
    weights: (B, S_q, S_k)  (non-negative, sum to 1 along -1 typically)
    values:  (B, S_k, D)
    returns: (B, S_q, D) aggregated Poincaré points
    Implementation: for each query position i, compute z = ⊕_j (w_ij ⊗ v_j)
    (iterative Möbius addition)
    """
    B, S_q, S_k = weights.shape
    _, S_k2, D = values.shape
    assert S_k == S_k2

    out = []
    for i in range(S_q):
        # weights for query i: (B, S_k, 1)
        w = weights[:, i, :].unsqueeze(-1)   # (B, S_k, 1)
        # scalar multiply each value by its weight -> (B, S_k, D)
        scaled = mobius_scalar_mul(w, values)  # broadcasting a has shape (B,S_k,1)
        # now reduce via Möbius addition
        z = scaled[:, 0, :]                         # initialize with first
        for j in range(1, S_k):
            z = mobius_add(z, scaled[:, j, :])
        out.append(z.unsqueeze(1))   # (B,1,D)

    return torch.cat(out, dim=1)   # (B, S_q, D)


def mobius_scalar_mul(a, x, eps=1e-5):
    """
    Möbius scalar multiplication: a ⊗ x in Poincaré ball.
    a: scalar or shape (...,1) (real)
    x: (..., n)
    Uses formula: a ⊗ x = tanh( a * atanh(||x||) ) * x / ||x||
    """
    # compute norms
    norm_x = torch.norm(x, dim=-1, keepdim=True).clamp_min(eps)
    atanh_norm = 0.5 * (torch.log1p(norm_x) - torch.log1p(-norm_x))  # atanh(norm_x)
    # numerically stable atanh alternative: torch.atanh if available, but implemented robustly here
    # compute scalar multiplication
    scaled = torch.tanh(a * atanh_norm)
    return (scaled / norm_x) * x